...
const SettingStoreModule: Module<State, any> = {
  ...
  getters: {
    ...
    // Null means 'not set by the user' — the caller falls back to the ini config.
    autocompleteKeywordCase(state) {
      const value = String(state.settings.autocompleteKeywordCase?.value ?? '').toLowerCase();
      return ['preserve', 'upper', 'lower'].includes(value) ? value : null;
    },
    autocompleteQuoteIdentifiers(state) {
      const value = String(state.settings.autocompleteQuoteIdentifiers?.value ?? '').toLowerCase();
      return ['auto', 'always'].includes(value) ? value : null;
    },
