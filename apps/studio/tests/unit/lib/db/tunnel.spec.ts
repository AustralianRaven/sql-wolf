...
/** Compares paths without assuming the platform's separator. */
...
describe("connectTunnel SSH agent handling (#4193)", () => {
  ...
  it("reads the private key file when useAgent is false and privateKey is set", async () => {
    ...
    expect(posixPath(mockReadFileSync.mock.calls[0][0])).toContain("/keys/keyfile");
  ...
  it("reads the bastion private key file when bastionMode is 'keyfile'", async () => {
    ...
    expect(mockReadFileSync).toHaveBeenCalledTimes(1);
    expect(posixPath(mockReadFileSync.mock.calls[0][0])).toContain("/keys/bastion_key");
  });
